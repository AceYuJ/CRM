...
class Product extends Common
{
     ...
     * @return [type]          [description]
     */
    function getWhere($req=array())
    {
    	$userModel = new \app\admin\model\User();
		$where = [];
		//时间段
		if(empty($req['type']) && empty($req['start_time'])){
            $req['type'] = 'month';
        }
        if(!empty($req['start_time'])){
        	$where['contract.order_date'] = array('between',array($req['start_time'],$req['end_time']));
        }else{
        	$create_time = getTimeByType($req['type']);
			if ($create_time) {
	            $where['contract.create_time'] = array('between',array($create_time[0],$create_time[1]));
	        }
        }
        if(!empty($req['category_id'])){
        	$where['product_category.category_id'] = array('eq',$req['category_id']);
        }
        $where['contract.check_status'] = array('eq',2);
		//员工IDS
		$map_user_ids = [];	
		if ($req['user_id']) {
			$map_user_ids = [$req['user_id']];
		} else {
			if ($req['structure_id']) {
                $map_user_ids = $userModel->getSubUserByStr($req['structure_id'], 2);
            }
		}
